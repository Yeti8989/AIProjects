...
function depth_limited_search(initial_state,depth_limit) {
  ...
  state = initial_state;
  let endState = null;
  if(!is_goal_state(state) && depth_limit != 0){
    let sucs = find_successors(state);
    let ctr = 0;
    while(ctr < sucs.length){
        endState = depth_limited_search(sucs[ctr].resultState, depth_limit - 1);
      if(endState != null){
        endState.actions.unshift(sucs[ctr].actionID);
        endState.states.unshift(sucs[ctr].resultState);
        return(endState);
      }
      if(ctr == sucs.length-1){
        return(null);
      }
      ctr++;
    }
