...
function is_goal_state(state) {
  ...
  // for(let i=0; i<3; i++){
  //   for(let j=0; j<3; j++){
  //     if(goal[i][j] != state.grid[i][j]){
  //       isEqual = false;   
  //     }
  //   }
  // }
  return(JSON.stringify(state.grid)== JSON.stringify(goal));/***Your code to check for goal state here!***/;
}
...
function find_successors(state) {
  ...
  for(let i = -1; i < 2; i = i+2){
    if((x[0] + i) >= 0 && (x[0]+i < 3)){
      ...
      let temp = newState.grid[x[0]][x[1]];
      newState.grid[x[0]][x[1]] = newState.grid[x[0] + i][x[1]];
      newState.grid[x[0] + i][x[1]] = temp;
      successors.push({
        actionID : (i<0 ? 1 : 2),
    ...
    if((x[1] + i) >= 0 && (x[1]+i < 3)){
      ...
      let temp = newState.grid[x[0]][x[1]];
      newState.grid[x[0]][x[1]] = newState.grid[x[0]][x[1] + i];
      newState.grid[x[0]][x[1] + i] = temp;
      successors.push({
      actionID : (i<0 ? 3 : 4),
      resultState : newState
   });
...
function find_zero_index(state) {
  for(let i=0; i<3; i++){
    for(let j=0; j<3; j++){
      if(state.grid[i][j] == 0){
        return([i,j]);   
